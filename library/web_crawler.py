...
def get_hitsinformation(query_url):
    ...
    news_hits_df = pd.DataFrame()
    news_hits_df['NEWS'] = 1
    
    all_and_news_df = pd.merge(all_hits_df, news_hits_df, on = ['URL', 'Date'], how = 'left')
    all_and_news_df['NEWS'].fillna(0)
